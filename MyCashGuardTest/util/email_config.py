...
class SendOtp:
    ...
    def generate_otp(self):
        '''Generate a random OTP of a given length.'''
        self.otp = ''.join(random.choices(string.digits, k=self.length))
        return self.otp

    @staticmethod
    def send_email(otp, recipient_email):
        '''Send the OTP to the recipient via email.'''
        sender_email = "sayedmahfuze@yahoo.com"  # Change this to your Yahoo email
        sender_password = "Sayed08@sabeeha"  # Change this to your Yahoo password or app-specific password
        smtp_server = "smtp.mail.yahoo.com"
        smtp_port = 587  # Standard port for sending mail with TLS
        ...
        try:
            ...
            server.starttls()  # Secure the connection using TLS
            server.login(sender_email, sender_password)  # Log in to your Yahoo account
            text = msg.as_string()
...
# Main program to generate OTP and send the email
if __name__ == "__main__":
    sendOtp = SendOtp()
    otp = sendOtp.generate_otp()  # Generate a 6-digit OTP
    recipient_email = "sayedmahfuze@gmail.com"  # Change this to the recipient's email
    sendOtp.send_email(otp, recipient_email)
"""